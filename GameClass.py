    ...
    def move(self):
        ...
        for block in self.map.blocks:
            ...
            if collide == "top":
                ...
                self.vel[1] = 0
                self.pos[1] = block[1] + block[3] + self.size[1]  # Align bottom of player with bottom of block
            elif collide == "left":
                self.vel[0] = 0
                self.pos[0] = block[0] - self.size[0]  # Align left side of player with left side of block
            elif collide == "right":
                self.vel[0] = 0
                self.pos[0] = block[0] + block[2] + self.size[0]  # Align right side of player with right side of block

        self.pos += self.vel

        self.vel[1] += 0.1  # Gravity effect
        if self.vel[1] > 5:  # Limit falling speed
            self.vel[1] = 5
        ...
        for i in range(self.size[0]):
            for j in range(self.size[1]):
                if (self.tiles[i][j]):
                    ...
                    j * self.tileSize:(j + 1) * self.tileSize] = numpy.array(self.tileImg)
    ...
    def view(self, player, map_img):
        ...
        if len(player.target) != 2:
            ...
            for i in range(5):
                ...
                map_img[int(dotx) - 1:int(dotx) + 1, int(doty) - 1:int(doty) + 1] = (255-i*20, 255-i*20, 255-i*20)
        else:
            pos = player.target
